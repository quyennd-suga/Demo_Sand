    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // ── GRAIN ──────────────────────────────────────────────────
                // Use world position directly (anchored to object) to avoid stretching
                ...
                // Scroll grains upward based on FillVelocity × time
                float scrollY = _Time.y * _FillVelocity * _Turbulence * 0.08;
                float2 scrolledUV = grainUV + float2(0.0, scrollY);

                float2 baseCell = floor(scrolledUV * _NoiseScale);
                float2 flowCell = floor(scrolledUV * _NoiseScale * 0.2);

                float rawFlowX = signedRandom(flowCell + float2(0.1, 0.2)) * _FlowSpread;

                float offsetX = round(rawFlowX * _FlowSpread * _Turbulence * 8.0);

                float2 grainCell = baseCell + float2(offsetX, 0.0);
                ...
                if (surfaceBand > 0.001)
                {
                    ...
                    float brightness = lerp(0.75, _SurfaceGrainBright, sv.y);
                    float3 surfGrainColor = _SandColor.rgb * brightness;
